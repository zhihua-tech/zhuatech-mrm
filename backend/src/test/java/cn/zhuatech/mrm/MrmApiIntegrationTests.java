...
class MrmApiIntegrationTests {
    ...
    @Test void adminCanAssessCampaignReadiness() throws Exception {
        mvc.perform(post("/api/admin/campaign-readiness").with(httpBasic("admin", "admin123"))
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"briefCompleteness\":100,\"assetCompleteness\":80,\"budgetApproved\":true,\"audienceReadiness\":90,\"legalApproved\":false,\"trackingReadiness\":70}"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.readinessScore").value(73))
